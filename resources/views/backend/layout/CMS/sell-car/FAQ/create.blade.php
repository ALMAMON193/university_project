...
@section('main-panel')
    <div class="row gap-4 rounded">
        ...
        <div class="col-12 p-4 rounded" style="border: solid 1px gray; background: white;">
            ...
            {{-- update hero text --}}
            <form action="{{ route('cms.car.page.faq.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <textarea style="height: 100px" class="form-control" id="answer" name="answer" placeholder="Anaser">{{ old('answer') }}</textarea>
                    @error('answer')
                        <p class="text-danger">{{ $message }}</p>
                    @enderror
                </div>
