...
@push('script')
    ...
    <script>
        ...
        const deleteAuction = (id) => {
            try {
                let url = `{{ route('backend.user-list.destroy', ':id') }}`;
            ...
            try {
                ...
                $.ajax({
                    url: `{{ route('backend.user-list.status.update') }}`,
                    method: 'PATCH',
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
            ...
            const auctionRoleHandler = (id, role) => {
            try {
                // console.log(id);
                let selected_state = $(role).val();
                console.log(selected_state);
                $.ajax({
                    url: `{{ route('backend.user-list.role.update') }}`,
                    method: 'PATCH',
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: id,
                    ...
                    success: function(response) {
                        ...
                        if (response.success === true) {
                            ...
                            var newClass = 'btn btn-sm dropdown-toggle ' + roleClassMap[response.role];

                            // Remove existing role classes
                            $(roleElement).removeClass(
                                'btn-success btn-secondary');

                            // Add the new role class
                            $(roleElement).addClass(newClass);
                            console.log(response.message);
            ...
            try {
                if (!$.fn.DataTable.isDataTable('#data-table')) {
                    let dTable = $('#data-table').DataTable({
                        ...
                        ajax: {
                            url: "{{ route('backend.user-list.index') }}",
                            type: "get",
                        },
